...
- ⚙️ 测试：新增 **56 条回归断言**（D1–D10 缺陷、E1–E5 事件索引、F1–F9 快照校验与校验时机、
  G1–G5 状态文件、H1–H4 度量、I1–I5 跨会话与成本、J1–J2 主页，外加 4 条演示录制不变量），全量以
  `python test_all.py` 的实际输出为准（本机 2310 / 2310 通过，跳过 3 项能力探测）。

...
| Container (optional) | `--sandbox docker`: one-shot container, `network none`, `cap-drop ALL`, `read-only` root, only the workspace mounted — the sandbox image is pulled from GHCR on first use (multi-arch, so Apple Silicon runs it natively) |
...
- **v3.11.0** (2026-09-19): the container tier is now prebuilt-first for Linux and macOS — `ghcr.io/ace-code-engine/ace-sandbox` is published multi-arch (native arm64 on Apple Silicon) with provenance and SBOM, and a missing image is pulled automatically instead of telling you to build it; a locally built image still wins. Supply-chain handles: `sandbox.image_digest` in every result, `--sandbox-image <ref>@sha256:<digest>` to pin, `ACE_SANDBOX_NO_PULL=1` to stay offline. Run flags hardened: `--init`, `--ulimit nofile`, `HOME=/tmp`, automatic `,z` on SELinux-enforcing hosts, and `--label` for cleanup. **Behaviour change**: the missing-image path now reaches the network unless you turn it off

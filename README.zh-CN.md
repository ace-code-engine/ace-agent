...
- **v3.13.0** (2026-09-19)：上下文余量从"压缩之后才知道"变成常驻可见。底栏末尾多一段占比（`上下文38%`），颜色即语义——灰=有余量、黄=用掉触发点的 80%、红=下一轮就会压缩；`/status` 打出明细（`约 12k tokens / 窗口 32768（38%，压缩触发点约 23k；估算值，不是服务端读数）`）。逼近阈值时在请求发出前提醒一次（按触发点 10% 一档节流，所以这行字一直值得看）。显示口径与实际压缩决策共用同一个策略构造点——各写一份迟早会对不上，而对不上的数没人会再信。守卫：`[9]` +17 条，含一条盯"触发点同源"的不变量

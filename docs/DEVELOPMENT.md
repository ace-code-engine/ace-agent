...
**两个补充（2026-09-26 发布 v3.43.0 时踩出来的）**：

1. **`--check` 只在 CI 的 3.12 job 上跑**（3.10/3.11 是 `skipped`），所以"图里的版本号还是旧版"
   这种错误**本地全量套件看不出来**：那次本地 2369/2369 全绿、ruff 零命中，CI 红在
   `demo_blocked.svg 里的版本号是 3.42.0`，而那一刻 tag 已经推上去、Release 正在建。
   现在 `test_all [68]` 也用录制脚本自己的 `svg_version()` 在本地钉一遍同一判据 ——
   升版本号不重录，本地就红（有反例验证：把一张图换回旧版本，[68] 当场点名是哪张、什么版本）。
2. **录制环境要把平台状态目录一起搬进临时 HOME**（`LOCALAPPDATA` / `APPDATA` /
   `XDG_STATE_HOME`），只搬 `HOME` 在 Linux 上够、在 Windows 上不够：签名锚（RG-01）读的是
   `%LOCALAPPDATA%`，于是锚落在**录制者自己的** AppData 里；受限环境下建不出来 → 写路径
   fail-close → `diff` 那张录到的是一条 `403 无法创建写前快照`，还把录制者的真实路径印进了
   发布出去的 SVG。搬进来之后四张图在本机与 CI 录出同一份骨架。

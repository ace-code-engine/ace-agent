...
2. **写前自动快照。** 每次写操作都留物理快照：`/snapshots` 看列表，`/undo` 回滚最近一次；`.guardian/` 目录 Agent 自己改不了。回滚范围**就是那一轮动过的路径** —— 你同时自己改的文件不会被一起退回去（H-08）。用 `terminal_exec` / `code_execute` 这类"能任意写盘"的工具时范围说不清，会退回整树还原：宁可多退，不可少退。

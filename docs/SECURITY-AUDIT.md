...
**这张表不再是"某一天的实测记录"**：能自动化的 payload 已经钉进 `test_all.py` 的 `[40] 安全审计 payload 回归`（SEC-003 / 005 / 006 / 007 / 010 / 014 / 018 / 019），随 CI 每轮跑。SEC-009、SEC-013、SEC-016、SEC-017 各自的断言分散在 test_all 的对应小节里（见上表"证据"列）。要推翻其中任一条，方法同「复审记录」：不看"改过没有"，只拿报告里的原始 payload 打当前代码。
...
- SEC-010 快照 HMAC 无配置入口：签名改为**默认启用**。密钥解析集中在 `guardian.resolve_signing_key()`（配置 > `ACE_SIGNING_KEY` > 密钥文件 > 自动生成），调用点放在 `ExecutionLayer.__init__` 而不是各 CLI 入口 —— 本条的成因正是"两个入口各写一遍、漏了一个"。三个配套判断：
  ...
  - **勘误（2026-09-26，RG-01 实施时逐条核对）**：上面这条记录描述的几处机制**在代码里并不存在** ——
    `guardian.resolve_signing_key()`、`ACE_SIGNING_KEY`、`~/.ace/snapshot_signing_key`、
  ...
  - **RG-01 已按本段的原则把它修掉**（2026-09-26，见 `docs/design/RGTC-LANDING.md`）：密钥移到工作区外的
    锚目录（`%LOCALAPPDATA%/ace-agent/state/<项目哈希>/`，可用 `ACE_ANCHOR_DIR` 或构造参数注入），
    老项目**迁移时把项目内那份删掉**（只拷不删等于没搬），锚不可用时 `snapshot()` 直接拒绝
    （fail-close，不生成未签名快照），带签名却拿不到密钥的快照**拒绝验证**而不是降级成只比摘要。
    同一探针：修复前"改内容+修摘要+重签" → `verify=True`；修复后 → `verify=False`（签名校验失败）。
  - 本段末尾那条前提（"`write`/`full` 下读得到密钥……那是整机失守，不是签名要解决的问题"）**不成立**：
    agent 持 `write` 正是**正常工作模式**，它够得着工作区是设计如此 —— 锚的意义恰恰是待在
    **工作区之外**。密钥放在项目内时，"读得到"不是极端情况，而是默认情况。

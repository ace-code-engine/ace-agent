...
- **项目外"已存在的东西"要问人**（v3.7 补）：绝对路径本身是"用户明确意图"——往桌面丢个**新**文件不问；但**覆盖或删除项目外已经在那儿的文件**会逐次确认，因为项目外没有快照可回滚，一次误写是永久的。确认按**路径**授权：会话级批准只记住那一条路径，换个文件还要问；命中敏感目标（凭据/私钥等）则直接 403，不给你一个"点了也没用"的确认框

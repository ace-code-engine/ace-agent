# 立项卡：安全边界加固 H-01 ~ H-26（全部已实施）

> 状态：**H-01 ~ H-26 全部实施并验收**（W0 ~ W7 + 独立工作包 H-08）。
> **本卡没有未决项。** 原"只剩 H-08：精确回滚"的那条收口见 §18。
...
> 本卡 §2 保留原始规划口径，§9 ~ §18 是实施记录（含与卡片不一致之处）。
...
| 项 | 状态 | 说明 |
|---|---|---|
| **H-08** 精确回滚 | **✅ 已实施** | 快照自带回滚范围（`rollback_scope`），说得清就精确、说不清才整树；顺带让 `/undo`、`/rollback <id>` 这两个事后入口也不再整树抹平。见 §18 |
...
## 18. H-08 实施记录：精确回滚（"只回滚本轮"从承诺变成事实）

§9.4 当时把它挂起，理由是"现有 5 条断言依赖整树还原语义"。实际动手后发现真正要先解决的是
**范围说不说得清**（见 18.2）—— 而下面 18.8 显示那 5 条断言一条都不用改。

### 18.1 先红：把 bug 复现出来（不是读代码推断）

`rollback()` 此前一律**整树还原**：删掉当前树里所有可收集文件，再按 `meta["files"]` 全量写回。
探针实测（读操作 + 临时目录，未碰仓库）：

| 时刻 | `agent.txt` | `user.txt` |
|---|---|---|
| 建快照时 | a1 | u1 |
| 本轮改 `agent.txt`；**用户同时**在编辑器里改 `user.txt` | a2-agent-round | u2-USER-EDIT |
| `rollback(sid)` 之后 | a1 ✓ | **u1 ✗** |

用户对**无关文件**的编辑被一起抹掉了 —— 而 `SECURITY-MODEL.md:36` 承诺的正是"只回滚本轮，
不动无关修改"。文档对、实现不对，这就是 H-08。

### 18.2 关键设计判断：**不能一律精确**

精确回滚的前提是"知道这一轮动了哪些路径"。实测 `WRITE_TOOLS` 有 **15 个**，只有 **4 个**说得清：

| | 工具 | 说得清吗 |
|---|---|---|
| `core.targets.WRITE_TOOLS_WITH_PATH` | `file_write` `str_replace` `file_delete` `file_move` | ✅ 唯一入口是 `core/targets.destructive_targets()`（H-13 建的那个） |
| 其余 11 个 | `terminal_exec` `code_execute` `subagent` `db_write` `image_generate` `kb_add` `api_post` `browser_*` `notify_send` | ❌ 能任意写盘 |

**若对这 11 个也走精确回滚，`touched_paths` 是空的 → 回滚报成功却什么都没做。**
一个会静默失效的安全机制，比一个多退一点的机制危险得多。所以定了方向：

> **宁可多退（用户看得见、备份还在、可恢复），不可少退（用户以为撤掉了，其实还在盘上）。**

这不是纸上推理：`[8]` 段"违规自动回滚（仅本轮快照）"用的正是 `terminal_exec` 造
`created.txt` —— 它当场就是"tree 档"的存在性证明，把这一档去掉它就红。

### 18.3 范围记在**快照**里，而不是靠调用方记得传

第一版实现是"调用方把范围传进 `rollback()`"。写到一半发现漏了一半场景：**`/undo` 与
`/rollback <id>` 是事后入口**，调用时早就没有本轮的 `ctx` 了 —— 范围若只走参数，这两个命令
仍然整树抹平，而它们恰恰是"快照可能很旧、用户必然有无关编辑"的场合。改成让快照自己记：

| 位置 | 行为 |
|---|---|
| `snapshot(tag, touched=None)` | 给了 `touched` → meta 记 `rollback_scope="paths"` + 归一后的 `touched`；否则记 `"tree"` |
| `rollback(snap_id, only=None)` | `only` 省略时**读 meta**：记的是 `paths` 就精确，否则整树 |
| 旧快照 | meta 缺这两个键 → 按 `tree` 处理，**改动前行为逐字不变** |
| `only=` | 仍保留显式覆盖（调用方现场知道得更多时用；也是测试的抓手） |

`/undo`、`/rollback <id>` **一行代码都没改**，自动从"整树"变成"精确"。

### 18.4 路径归一：必须走 H-10 那个入口

`touched` 是**模型写在参数里的原始串**（可能带 `..`、8.3 短名、大小写差异、绝对路径）。
不归一就和 `meta["files"]` 的键对不上，后果不是报错而是**静默不做事**。

顺带查出一个真缺陷：`canonical_path()` 把相对路径的起点写死成 `Path.cwd()`。而"模型参数里的
相对路径"起点是**项目根** —— 两者在 `--project-root` 与 cwd 不同的场景下（无头、测试）并不
相等，用 cwd 会把项目内的文件解析到项目外。已加 `base=` 参数（默认 `None` = 原行为）。

项目外路径：不在快照里、无法从快照恢复、也不该在这里删 → 跳过，并在 stderr 明说
"有 N 个在项目外，快照盖不住它们"（不静默）。

### 18.5 顺手消掉的一个隐患

旧实现是"先删掉所有文件、再全量恢复"；中间那一瞬失败，连快照里有的文件也会停在
"已删除、未恢复"。新实现把动作折成两个集合（**要还原的** / **要删的**），快照里有的文件
**不再先被删** —— `SECURITY-AUDIT.md:837` 记的那个窗口因此消失。

### 18.6 改动

| 文件 | 改动 |
|---|---|
| `core/guardian.py` | 新增 `_relative_targets()`；`snapshot(tag, touched=None)`；`rollback(snap_id, only=None)` 重写为"两个集合"模型；meta 新增 `rollback_scope` / `touched` |
| `core/canonical.py` | `canonical_path(path, *, base=None)` —— 相对路径的起点可指定（默认仍 cwd） |
| `execution_layer.py` | `RoundCtx` 新增 `rollback_scope` / `touched_paths`；`_stage_snapshot` 定范围并写进快照；`_rollback_current_snapshot(snapshot_id, only=None)` |
| `test_all.py` | `[70]` 新增 7 条 H-08 断言（含端到端"执行层确实把范围写进了快照"）；`[10]` 新增 2 条 `/undo` 精确回滚断言 |
| 文档 | `SECURITY-MODEL.md:36`（那句承诺现在是**对的**，并写清 tree 档与方向）、`INTERFACES.md:138`（契约签名）、`GETTING-STARTED.md:50`、`guardian.py` 模块 docstring |

**公开契约变更**（引用者需同步）：`Guardian.snapshot()` / `Guardian.rollback()` 各多一个
**可选**参数（老调用一行都不用改）；快照 meta 多两个键（旧快照缺键 → 整树，向后兼容）。

### 18.7 两面对照（不只看它绿，还看它会不会红）

把 `_stage_snapshot` 里传 `touched` 的那半**临时停掉**，重跑 `[10]`：

```
❌ H-08 ★★/undo 不再抹掉用户对无关文件的编辑（H-08 的验收点）  my notes
通过 505 / 506    exit=1
```

失败详情正是 `my notes`（用户的编辑被退回成快照时的内容）。恢复后 506 / 506。
⇒ 这条断言**真的在量 H-08**，不是空守卫。

### 18.8 验收

| 项目 | 结果 |
|---|---|
| 全量测试 | **2256 / 2256 全绿 · 退出码 0**（上一轮 2247；净 +9 条 H-08 断言） |
| `[70]` H-08 | 7 条全 ✅（含两条端到端：`file_write` 轮记 `paths`、`terminal_exec` 轮记 `tree`） |
| `[10]` H-08 | 2 条全 ✅（`/undo` 走精确回滚） |
| 既有回滚断言 | `[3]` 整树、`[8]` 违规自动回滚、`[10]` `/undo`、SEC-04（签名后回滚）、SEC-016（逐项容错）—— **一条都没改，全部仍绿** |
| `ruff`（CI 口径） | All checks passed |
| 非空性 | 见 §18.7（负向实测到红） |

§9.4 担心的"5 条断言依赖整树还原"**没有成为问题**：它们要么不传 `touched`（→ 旧快照缺键 →
整树，逐字不变），要么走 `terminal_exec`（→ 本来就在 tree 档）。**兼容性来自设计，不是靠改测试。**

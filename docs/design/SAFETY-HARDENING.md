...
> 状态：**W0 + W1 + W2 + W3 + W4 + W5 + W6 已实施**（H-01 ~ H-07、H-09 ~ H-21、H-23、H-24、H-25 完成并验收；**H-08 未做**，理由见 §9.4）。
> **只剩 H-08（需独立包）、H-22（依赖契约，需你拍板）、H-26（测试污染，独立小项）**。
> 全量测试：**2240 / 2240 全绿、退出码 0，零失败项，且不需要 `PYTHONUTF8=1` 这类环境变量**。
> 本卡 §2 保留原始规划口径，§9 ~ §14 是实施记录（含与卡片不一致之处）。
> 本卡 §2 保留原始规划口径，§9 是实施记录（含与卡片不一致之处）。
> 编号：`H-` 是**新命名空间**，不与 `SEC-` / `Q-` / `R-` / `REL-` 共享编号空间（吸取 ADR-002 与独立 ADR 文件撞号的教训）。
> 来源：2026-09-26 三路独立只读审计（执行层安全 / agent 循环与协议 / 前端与扩展面），共 38 条；其中 20 余条经复核，1 条被推翻（见 §7）。
...
### 10.5 H-26：测试把会话写进仓库自己的 `.ace_sessions/`（**未修**，已量化）
...
| **H-26** 测试污染 `.ace_sessions` | **未修** | +8 文件/轮（既有测试所致；我只修了自己新增的两处），见 §10.5 |
| 那 4 条 `--serve` 失败 | **已修** | 根因是 H-24，不是环境噪音，见 §9.8 |

**验收总账**：全量 **2240 / 2240 全绿 · 退出码 0 · 零环境变量要求**；`ruff` 全过；
`[38]` 结构守卫全绿。**全部改动仍未 commit**（本仓库纪律是"每步一个独立快照、可单独
`git revert`"，提交时机由你定）。

...
退出码 0、stdout 含最终回复标记 🤖 Agent: 且回复非空。

抗抖动（Q-08）：最多 3 次尝试，**每次 150s 超时**（不是一次 240s 硬超时）——
第 1 次是带工具调用的完整提问，后两次换成不需要工具的浅提问，降低单点失败面。
尝试次数可用 `ACE_E2E_ATTEMPTS` 调；任何一次成功即通过，全部失败才以 1 退出。

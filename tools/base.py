...
class ToolExecutorBase:
    ...
    def _read_text_exact(path: Path) -> Tuple[str, str]:
        ...
        回退编码用的是 `locale.getpreferredencoding(False)`，因此**行为随机器而变**：
        中文 Windows 上是 cp936（GBK 源码能解开），英文 Windows / CI runner 上是 cp1252
        （GBK 源码解不开，于是走"拒绝改写"那条路）。这是刻意的选择，也是被测试记录下来的
        行为——它意味着"同一个 GBK 文件在两台机器上命运不同"，但两边都**安全**：
        要么按原编码安全改写，要么明确拒绝，不会静默重编码。

        这里曾经想再加一道"解码后能否逐字节还原"的校验，用来拦"解得开却编不回"的编码。
        实测表明那是多余的：CPython 的单字节编解码器要么无损往返，要么直接解码失败
        （cp1252 的 0x81/0x8D/0x8F/0x90/0x9D 就属于后者），并不存在"解得开却编不回"
        的中间状态。所以没有加——守卫要盯真实成立的东西，不是想象出来的风险。
        """
        try:
